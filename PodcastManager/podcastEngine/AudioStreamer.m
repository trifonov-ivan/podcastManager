...
void MyAudioQueueInputCallback(void *aqRecorderState,
                               AudioQueueRef                        inQ,
...
{
    //this is called by the audio queue when it get audio buffer
...
- (void)createQueue
{
    ...
    UInt32 fmt = asbd.mFormatID;
    
//    err = AudioQueueNewInput(&asbd, MyAudioQueueInputCallback, self, NULL, NULL, 0, &audioQueue);
//	if (err)
//	{
//		[self failWithErrorCode:AS_AUDIO_QUEUE_CREATION_FAILED];
//		return;
	...
	packetDescriptions:(AudioStreamPacketDescription *)inPacketDescriptions;
{
    ...
    {
        AudioFileWritePackets(mAudioFile, NO, inNumberBytes, inPacketDescriptions, mRecordPacket, &inNumberPackets, inInputData);
        mRecordPacket +=inNumberPackets;
    }
...
{
    return (mAudioFile != NULL);
}
- (BOOL)setFileToWrite:(NSString*) path
{
    const UInt8 *pFilepath = (const UInt8 *)[path UTF8String];
